...
class LocalInference(ABC):
    ...
    Subclasses adapt a specific backbone (Pi3/Pi3X, VGGT, MapAnything,
    VGGT-Omega) to a shared :meth:`predict` interface returning depth,
    confidence, extrinsics, and intrinsics.
    """
...
def create_local_inference(
    ...
    if model_type in ("pi3", "pi3x"):
        ...
        from gluemap.ff_inference.vggt_omega_inference import (
            VGGTOmegaLocalInference,
        )

        return VGGTOmegaLocalInference(model, device, dtype)
    else:
